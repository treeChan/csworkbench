; ============================================================
; 自定义 NSIS 模板(基于 tauri-bundler dev 分支默认 installer.nsi)。
; 改动:同版本重装 / 版本升级时强制「覆盖安装」,跳过「先卸载再安装」
;       选择页,保证升级不卸载、appdata 配置与数据原样保留。
; 维护:升级 Tauri 版本时,对照新版 tauri-bundler 模板同步此文件
;       (重点是 PageReinstall / PageLeaveReinstall 两处「自定义」块)。
; ============================================================
Unicode true
...
Function PageReinstall
  ...
  Pop $R0
  ; ---- 自定义:同版本重装 / 版本升级 → 直接覆盖安装,不弹「卸载/不卸载」选择页。
...
Function PageLeaveReinstall
  ...
  ; ---- 自定义:同版本重装 / 版本升级一律覆盖安装(双保险,即使走到这里也不卸载)。
  ${If} $R0 >= 0
    Goto reinst_done
  ${EndIf}
